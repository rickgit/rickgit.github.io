...
+------------------------------------------------------------------------------------------------------+
|Trust                         |                  |  Spring boot ,jenkins                              |稳定（单元测试）
|JaCoCo/Cobertura              |                  |  AutoTools(gnu make), Cmake ,Ninja,SCons           |自动化（编译，测试，调试）
|Junit,mockito/ EasyMock,Gtest,|  vss             |  cruisecontrol,APT(dagger asm/CGLIB )              |可分析（调试，日志，安全）
|TestNT/CPPUnit                | GIT,Hg,svn,fossl | √ Ant/ivy ,√Maven,Groovy(DSL)/Gradle,git submodule |高效（稳定自动化，可分析可拓展）
+------------------------------------------------------------------------------------------------------+
| unit test                    | version control  |     Pragmatic Project Automation                   |Efficiency,Stability，scalable 
+------------------------------------------------------------------------------------------------------+

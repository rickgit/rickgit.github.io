...
+------------------------------------------------------------------------------------------------------+
|      kernel (POSIX)                                      audio               Graphics                |✨高效-避免重复迭代（稳定自动化，可拓展可分析）
+--------------------------------------------------------------------------+---------------------------+✨省时-粒度小避免耗时（算法运行快，低延迟，高吞吐）
|Apache(commons-cli )|                                       |             | GreenDB,Cupboard,Redis    |✨体验（流畅，美观（排版，适配，卡顿），省电）
| OOAD               |                                       |APKProtect   +---------------------------+
...
|Junit,mockito/ EasyMock,Gtest,|  cruisecontrol,APT(dagger asm/CGLIB )              |  vss             |稳定（代码安全性，可读性，单元测试）
